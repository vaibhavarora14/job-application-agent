...
Hosted attention resume (P1 site coordination, not a second ledger): while an attention item is open and the lease is held, poll with `node job-application-agent/scripts/attention-runner-poll.mjs --attention-id …` (Bearer `ATTENTION_NOTIFY_SECRET` against `GET {PUBLIC_SITE_URL}/api/internal/attention-signals/:id`). Signals are `resume_requested`, `skipped`, or `aborted`. Live session entry is `GET /api/attention/:id/live-session?token=…` (Worker embed/redirect to noVNC, or soft unavailable when base URL unset — never email VNC passwords; IAP helpers are founder/dev-only). After resume: renew lease, re-inspect the ATS page, submit only on visible confirm. See `site/docs/ATTENTION.md`. Never store passwords, MFA codes, CAPTCHA answers, or session cookies in D1/KV.
