...
export function PaymentReturnStatus() {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      ...
      if (!registrationId) { if (active) setView("failed"); return; }
      attempts += 1;
      try {
        const response = await fetch(`/api/checkout/status?registration_id=${encodeURIComponent(registrationId)}`, { cache: "no-store" });
  ...
  return <div className="payment-return-card" aria-live="polite">
    ...
    {view === "paid" && <><span className="success-mark">✓</span><p className="eyebrow">Payment verified</p><h1>Your founding place is reserved.</h1><p>We&apos;ll email onboarding details before the first cloud run.</p></>}
    {view === "processing" && <><p className="eyebrow">Still processing</p><h1>Your payment is still being confirmed.</h1><p>It is safe to close this page. We&apos;ll update your registration when confirmation arrives.</p></>}
