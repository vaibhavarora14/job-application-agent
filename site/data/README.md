...
- Compensation is extracted directly from ATS postings (Ashby `descriptionPlain`, Greenhouse `content`, Lever `additionalPlain`, Workable `salary`/`description`) only when explicitly stated by the employer in the job application. When omitted by the employer, salary is left blank—no estimates or assumed benchmarks are generated.
