...
export function PublishedJobs({ locationIndex }: { locationIndex: LocationIndex }) {
  ...
  const clear = () => { setQuery(""); setCompany(""); setChannel(""); setLocation(""); setCountry(""); setWorkplace(""); setVisible(25); };
  ...
  return <main className={`page-width ${styles.page}`}>
    ...
    <section className={styles.filters} aria-label="Search and filter published jobs">
      ...
      <div className={styles.filterRow}>
        <label>Location<input type="search" aria-label="Location" placeholder="City, region, or country…" value={location} disabled={loading || error} onChange={event => { setLocation(event.target.value); setVisible(25); }} /></label>
        <label>Country / region<select aria-label="Country / region" value={country} disabled={loading || error} onChange={event => { setCountry(event.target.value); setVisible(25); }}><option value="">All countries / regions</option>{countries.map(value => <option key={value} value={value}>{countryLabel(value)}</option>)}</select></label>
        <label>Work arrangement<select aria-label="Work arrangement" value={workplace} disabled={loading || error} onChange={event => { setWorkplace(event.target.value); setVisible(25); }}><option value="">Any arrangement</option><option value="remote">Remote</option><option value="hybrid">Hybrid</option><option value="onsite">On-site</option><option value="unknown">Not specified</option></select></label>
      </div>
      <div className={styles.filterRow}>
