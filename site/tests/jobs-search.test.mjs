...
test('combines location, country and workplace without treating unknown jobs as remote', () => {
  const jobs = [{ ...job(1), location: { label: 'New York', cities: ['New York'], countries: ['US'], workplace: 'remote' } }, job(2)];
  assert.equal(searchJobs(jobs, { location: 'united states', workplace: 'remote' }).length, 1);
  assert.equal(searchJobs(jobs, { location: 'London', workplace: 'remote' }).length, 0);
  assert.equal(searchJobs(jobs, { country: 'GB' }).length, 0);
  assert.equal(searchJobs(jobs, { workplace: 'unknown' })[0].jobId, job(2).jobId);
  assert.equal(searchJobs(jobs, { query: 'New York' }).length, 1);
});
